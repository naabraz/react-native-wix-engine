...
class IOSBuilder {
  constructor() {
    ...
    this._engineIosDir = `${
      this._repoDir
    }/internal_folder/react-native-wix-engine/ios`;
    this._workspacePath = `${
      this._engineIosDir
    }/ReactNativeWixEngine.xcworkspace`;
    this._xcodebuildTargetDir = `${
      this._repoDir
  ...
  _copyToAppBuilds(platform, buildType, buildFlavor) {
    const source = `${
      this._engineIosDir
    }/DerivedData/ReactNativeWixEngine/build/Products/${
      buildFlavor.flavorDir
    }-${platform.name}/ReactNativeWixEngine.${platform.ext}`;
    const destinationDir = `${
      this._repoDir
    }/internal_folder/react-native-wix-engine/app_builds/${
      platform.name
    }/${buildType}`;
  ...
  _extractIpa(buildType, version) {
    ...
    const archivePath = this._archivePath(buildFlavor.scheme, version);
    const tempDir = execSync('mktemp -d /tmp/EngineBuild.XX')
      .toString()
      .trim();
    ...
    const finalBinaryDirectory = `"${
      this._repoDir
    }/internal_folder/react-native-wix-engine/app_builds/iphoneos/${buildType}"`;
    execSync(`mkdir -p ${finalBinaryDirectory}`);
    execSync(
      `cp -v "${tempDir}/${
        buildFlavor.scheme
      }.ipa" ${finalBinaryDirectory}/ReactNativeWixEngine.ipa`,
    );
